...
1. Model.select():
   - Most flexible, creates SELECT * FROM table
   - Returns ModelSelect (iterable of model instances)
   - Can chain .where(), .order_by(), .join(), etc.
   - Use for: Getting multiple records

2. Model.get_by_id(pk):
   - Fetches single record by primary key
   - Returns model instance or raises DoesNotExist
   - Use for: When you have the primary key

3. Model.get_or_none(condition):
   - Fetches single record matching condition
   - Returns model instance or None
   - Use for: Safe single record queries

4. Model.select(fn.COUNT(Model.id)).scalar():
   - Aggregate queries
   - .scalar() returns a single value
   - Use for: COUNT, SUM, AVG queries

Example Workflow:

# 1. Define models (this file - models.py)
class User(BaseModel):
    id = AutoField()
    username = CharField(max_length=50)

# 2. Create tables (run once, usually in setup script)
#    db.create_tables([User, Task, Tag, TaskTag])
